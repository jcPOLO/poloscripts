...
# Regex
RE_CE_SERIAL_VERSION = r"\((?P<serial_number>CE\S+)\s+(?P<os_version>V\S+)\)"
    ...
    def get_facts(self):
        ...
        for line in show_ver.splitlines():
            if 'VRP (R) software' in line:
                search_result = re.search(RE_CE_SERIAL_VERSION, line)

                # TODO: for S2700
                search_qw = re.search(RE_QW_OS_VERSION, line)
                ...
                # TODO: for S2700   
                elif search_qw is not None:
                    os_version = search_qw.group('os_version')
                    # serial number in elabel command.z
